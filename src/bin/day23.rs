...
fn find_dst(cur: usize, pickup: &Vec<usize>, max: usize) -> usize {
    ...
    candidates[1] = if candidates[0] == 1 {max} else {candidates[0]-1};
    candidates[2] = if candidates[1] == 1 {max} else {candidates[1]-1};
    candidates[3] = if candidates[2] == 1 {max} else {candidates[2]-1};
